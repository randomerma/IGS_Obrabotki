...
Функция СоздатьТабличныйДокументЗРДС()
	...
	//Если НЕ СсылкаНаОбъект.СтатьяДДС.ПринадлежитЭлементу(Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Налоги", Истина)) 
	//	И НЕ СсылкаНаОбъект.СтатьяДДС.ПринадлежитЭлементу(Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Налоги и сборы (с ФОТ)", Истина)) Тогда
	//	ВывестиДКЗ(ТабДокумент, Макет);			
	//КонецЕсли;	
	//
	//// Вывод ДКЗ
	//Если СсылкаНаОбъект.СтатьяДДС.ПринадлежитЭлементу(Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Аренда ОС", Истина)) Тогда
...
Процедура ВывестиДКЗ(ТабДокумент, Макет)
	...
	|	СУММА(0) КАК РегНачОстатокДт,
	|	СУММА(0) КАК РегНачОстатокКт,
	|	СУММА(ВЫБОР
	|			КОГДА ЗачетАванса.СуммаОборотДт >= 0
	|				ТОГДА ВЫБОР
	|						КОГДА ЗачетАванса.СуммаОборотДт - ЗачетАванса.СуммаОборотКт < 0
	|							ТОГДА ЗачетАванса.СуммаОборотДт
	|						ИНАЧЕ ЗачетАванса.СуммаОборотКт
	|					КОНЕЦ
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ЗачетАванса.СуммаОборотДт - ЗачетАванса.СуммаОборотКт > 0
	|						ТОГДА ЗачетАванса.СуммаОборотДт
	...
	|				КОНЕЦ
	|		КОНЕЦ) КАК РегРасход,
	|	СУММА(0) КАК РегКонОстатокДт,
	|	СУММА(0) КАК РегКонОстатокКт,
	|	ЗачетАванса.Регистратор КАК Регистратор,
	|	СУММА(0) КАК РегНачОстатокДтВал,
	|	СУММА(0) КАК РегНачОстатокКтВал,
	|	СУММА(ВЫБОР
	...
	|	СУММА(0) КАК РегКонОстатокДтВал,
	|	СУММА(0) КАК РегКонОстатокКтВал
	...
	|			) КАК ЗачетАванса
	|ГДЕ
	|	ЗачетАванса.СуммаОборотДт <> 0
	|	И ЗачетАванса.СуммаОборотКт <> 0
	|	И НЕ ЗачетАванса.Регистратор ССЫЛКА Документ.ПереоценкаВалютныхСредств
	|	И НЕ ЗачетАванса.Регистратор ССЫЛКА Документ.ОтражениеЗарплатыВРеглУчете
	|	И ВЫБОР
	|			КОГДА ЗачетАванса.Счет = &Счет60
	|				ТОГДА ЗачетАванса.Субконто1 = &Контрагент
	|			КОГДА ЗачетАванса.Счет В (&Счета6002_6201)
	|				ТОГДА ЗачетАванса.Субконто1 = &Контрагент
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПриобОбороты.Организация,
	|	ПриобОбороты.СчетОплаты,
	|	ПриобОбороты.Контрагент,
	|	ПриобОбороты.ДоговорКонтрагента,
	|	ПриобОбороты.Документ,
	|	0 КАК РегНачОстатокДт,
	|	0 КАК РегНачОстатокКт,
	|	ПриобОбороты.СуммаРегПриход КАК РегПриход,
	|	ПриобОбороты.СуммаРегРасход КАК РегРасход,
	|	0 КАК РегКонОстатокДт,
	|	0 КАК РегКонОстатокКт,
	|	ПриобОбороты.Регистратор,
	|	0 КАК РегНачОстатокДтВал,
	|	0 КАК РегНачОстатокКтВал,
	|	ВЫБОР
	|		КОГДА ПриобОбороты.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ПриобОбороты.СуммаВзаиморасчетовПриход
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК РегПриходВал,
	|	ВЫБОР
	|		КОГДА ПриобОбороты.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ПриобОбороты.СуммаВзаиморасчетовРасход
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК РегРасходВал,
	...
	|			Запись,
	|			Организация = &Организация
	|				И СчетОплаты В ИЕРАРХИИ (&Счета_60_62)
	|				И ВЫБОР
	|					КОГДА СчетОплаты = &Счет60 ИЛИ СчетОплаты = &Счет6021 ИЛИ СчетОплаты = &Счет6031 ИЛИ СчетОплаты = &Счет76072
	|						ТОГДА Контрагент = &Контрагент
	|					КОГДА СчетОплаты В (&Счета6002_6201)
	|						ТОГДА Контрагент = &Контрагент
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК ПриобОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПриобНачОст.Организация,
	|	ПриобНачОст.СчетОплаты,
	|	ПриобНачОст.Контрагент,
	|	ПриобНачОст.ДоговорКонтрагента,
	|	ПриобНачОст.Документ,
	|	ВЫБОР
	|		КОГДА ПриобНачОст.СуммаРегОстаток > 0
	|			ТОГДА ПриобНачОст.СуммаРегОстаток
	|		ИНАЧЕ 0 * ПриобНачОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ПриобНачОст.СуммаРегОстаток > 0
	|			ТОГДА 0 * ПриобНачОст.СуммаРегОстаток
	|		ИНАЧЕ -ПриобНачОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	0,
	|	0,
	|	0,
	|	0,
	|	ЕСТЬNULL(""Рег"", ПриобНачОст.Организация),
	|	ВЫБОР
	|		КОГДА ПриобНачОст.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ВЫБОР
	|					КОГДА ПриобНачОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА ПриобНачОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ 0 * ПриобНачОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ПриобНачОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА 0 * ПриобНачОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ -ПриобНачОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	0,
	...
	|				И СчетОплаты В ИЕРАРХИИ (&Счета_60_62)
	|				И ВЫБОР
	|					КОГДА СчетОплаты = &Счет60 ИЛИ СчетОплаты = &Счет6031 ИЛИ СчетОплаты = &Счет6021 ИЛИ СчетОплаты = &Счет76072
	|						ТОГДА Контрагент = &Контрагент
	|					КОГДА СчетОплаты В (&Счета6002_6201)
	|						ТОГДА Контрагент = &Контрагент
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК ПриобНачОст
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПриобКонОст.Организация,
	|	ПриобКонОст.СчетОплаты,
	|	ПриобКонОст.Контрагент,
	|	ПриобКонОст.ДоговорКонтрагента,
	|	ПриобКонОст.Документ,
	|	0,
	|	0,
	|	0,
	|	0,
	|	ВЫБОР
	|		КОГДА ПриобКонОст.СуммаРегОстаток > 0
	|			ТОГДА ПриобКонОст.СуммаРегОстаток
	|		ИНАЧЕ 0 * ПриобКонОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ПриобКонОст.СуммаРегОстаток > 0
	|			ТОГДА 0 * ПриобКонОст.СуммаРегОстаток
	|		ИНАЧЕ -ПриобКонОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ЕСТЬNULL(""Рег"", ПриобКонОст.Организация),
	|	0,
	|	0,
	|	0,
	|	0,
	|	ВЫБОР
	|		КОГДА ПриобКонОст.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ВЫБОР
	|					КОГДА ПриобКонОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА ПриобКонОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ 0 * ПриобКонОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ПриобКонОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА 0 * ПриобКонОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ -ПриобКонОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|ИЗ
	...
	|						ТОГДА Контрагент = &Контрагент
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК ПриобКонОст
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РеалОбороты.Организация,
	|	РеалОбороты.СчетОплаты,
	|	РеалОбороты.Контрагент,
	|	РеалОбороты.ДоговорКонтрагента,
	|	РеалОбороты.Документ,
	|	0,
	|	0,
	|	РеалОбороты.СуммаРегРасход,
	|	РеалОбороты.СуммаРегПриход,
	|	0,
	|	0,
	|	РеалОбороты.Регистратор,
	|	0,
	|	0,
	|	ВЫБОР
	|		КОГДА РеалОбороты.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА РеалОбороты.СуммаВзаиморасчетовРасход
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА РеалОбороты.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА РеалОбороты.СуммаВзаиморасчетовПриход
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	0,
	|	0
	...
	|						ТОГДА Контрагент = &Контрагент
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК РеалОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РеалНачОст.Организация,
	|	РеалНачОст.СчетОплаты,
	|	РеалНачОст.Контрагент,
	|	РеалНачОст.ДоговорКонтрагента,
	|	РеалНачОст.Документ,
	|	ВЫБОР
	|		КОГДА РеалНачОст.СуммаРегОстаток > 0
	|			ТОГДА 0 * РеалНачОст.СуммаРегОстаток
	|		ИНАЧЕ -РеалНачОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА РеалНачОст.СуммаРегОстаток > 0
	|			ТОГДА РеалНачОст.СуммаРегОстаток
	|		ИНАЧЕ 0 * РеалНачОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	0,
	|	0,
	|	0,
	|	0,
	|	ЕСТЬNULL(""Рег"", РеалНачОст.Организация),
	|	ВЫБОР
	|		КОГДА РеалНачОст.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ВЫБОР
	|					КОГДА РеалНачОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА 0 * РеалНачОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ -РеалНачОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|			ТОГДА ВЫБОР
	|					КОГДА РеалНачОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА РеалНачОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ 0 * РеалНачОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	0,
	...
	|						ТОГДА Контрагент = &Контрагент
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК РеалНачОст
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РеалКонОст.Организация,
	|	РеалКонОст.СчетОплаты,
	|	РеалКонОст.Контрагент,
	|	РеалКонОст.ДоговорКонтрагента,
	|	РеалКонОст.Документ,
	|	0,
	|	0,
	|	0,
	|	0,
	|	ВЫБОР
	|		КОГДА РеалКонОст.СуммаРегОстаток > 0
	|			ТОГДА 0 * РеалКонОст.СуммаРегОстаток
	|		ИНАЧЕ -РеалКонОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА РеалКонОст.СуммаРегОстаток > 0
	|			ТОГДА РеалКонОст.СуммаРегОстаток
	|		ИНАЧЕ 0 * РеалКонОст.СуммаРегОстаток
	|	КОНЕЦ,
	|	ЕСТЬNULL(""Рег"", РеалКонОст.Организация),
	|	0,
	|	0,
	|	0,
	|	0,
	|	ВЫБОР
	|		КОГДА РеалКонОст.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &Руб
	|			ТОГДА ВЫБОР
	|					КОГДА РеалКонОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА 0 * РеалКонОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ -РеалКонОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|			ТОГДА ВЫБОР
	|					КОГДА РеалКонОст.СуммаВзаиморасчетовОстаток > 0
	|						ТОГДА РеалКонОст.СуммаВзаиморасчетовОстаток
	|					ИНАЧЕ 0 * РеалКонОст.СуммаВзаиморасчетовОстаток
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|ИЗ
	...
	|	ВТ_ЗачетАванса.РегНачОстатокДт,
	|	ВТ_ЗачетАванса.РегНачОстатокКт,
	|	ВТ_ЗачетАванса.РегПриход,
	|	ВТ_ЗачетАванса.РегРасход,
	|	ВТ_ЗачетАванса.РегКонОстатокДт,
	|	ВТ_ЗачетАванса.РегКонОстатокКт,
	|	ВТ_ЗачетАванса.Регистратор,
	|	ВТ_ЗачетАванса.РегНачОстатокДтВал,
	|	ВТ_ЗачетАванса.РегНачОстатокКтВал,
	|	ВТ_ЗачетАванса.РегПриходВал,
	|	ВТ_ЗачетАванса.РегРасходВал,
	|	ВТ_ЗачетАванса.РегКонОстатокДтВал,
	|	ВТ_ЗачетАванса.РегКонОстатокКтВал
	|ИЗ
	...
	|			,
	|			,
	|			Счет В ИЕРАРХИИ (&Счет76) И Счет <> &Счет76072,
	|			,
	|			Организация = &Организация
	...
	|	МИНИМУМ(СчетФактураПолученный.Ссылка) КАК Ссылка,
	...
	|	МИНИМУМ(СчетФактураВыданный.Ссылка),
	...
	|	ВТ_Результат.СчетОплаты КАК СчетОплаты,
	...
	|	СУММА(ВТ_Результат.РегНачОстатокДт) КАК РегНачОстатокДт,
	|	СУММА(ВТ_Результат.РегНачОстатокКт) КАК РегНачОстатокКт,
	|	СУММА(ВТ_Результат.РегПриход) КАК РегПриход,
	|	СУММА(ВТ_Результат.РегРасход) КАК РегРасход,
	|	СУММА(ВТ_Результат.РегКонОстатокДт) КАК РегКонОстатокДт,
	|	СУММА(ВТ_Результат.РегКонОстатокКт) КАК РегКонОстатокКт,
	|	СУММА(ВТ_Результат.РегНачОстатокДтВал) КАК РегНачОстатокДтВал,
	|	СУММА(ВТ_Результат.РегНачОстатокКтВал) КАК РегНачОстатокКтВал,
	|	СУММА(ВТ_Результат.РегПриходВал) КАК РегПриходВал,
	|	СУММА(ВТ_Результат.РегРасходВал) КАК РегРасходВал,
	|	СУММА(ВТ_Результат.РегКонОстатокДтВал) КАК РегКонОстатокДтВал,
	|	СУММА(ВТ_Результат.РегКонОстатокКтВал) КАК РегКонОстатокКтВал,
	|	ЕСТЬNULL(ВТ_Результат.Документ.ИГС_ПризнакНаличияОригинала, """") КАК Копия_Оригинал,
	|	ЕСТЬNULL(ВТ_СФ.ДатаСФ, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСФ,
	|	ЕСТЬNULL(ВТ_СФ.НомерСФ, """") КАК НомерСФ,
	|	ЕСТЬNULL(ДокументПТиУ.НомерВходящегоДокумента, """") КАК НомерВходящегоДокумента,
	|	ЕСТЬNULL(ДокументПТиУ.ДатаВходящегоДокумента, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаВходящегоДокумента
	|ПОМЕСТИТЬ ВТДанные
	|ИЗ
	|	ВТ_Результат КАК ВТ_Результат
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблСчФ КАК ВТ_СФ
	|		ПО ВТ_Результат.Документ = ВТ_СФ.ДокументОснование
	|			И (ВЫБОР
	|				КОГДА ВТ_Результат.СчетОплаты В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками))
	|					ТОГДА ВТ_СФ.Ссылка ССЫЛКА Документ.СчетФактураПолученный
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|			И ВТ_Результат.Контрагент = ВТ_СФ.Контрагент
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг КАК ДокументПТиУ
	|		ПО ВТ_Результат.Документ = ДокументПТиУ.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_Результат.СчетОплаты,
	|	ВТ_Результат.ДоговорКонтрагента,
	|	ВТ_Результат.Организация,
	|	ВТ_Результат.Контрагент,
	|	ЕСТЬNULL(ВТ_Результат.Документ.ИГС_ПризнакНаличияОригинала, """"),
	|	ЕСТЬNULL(ВТ_СФ.НомерСФ, """"),
	|	ЕСТЬNULL(ДокументПТиУ.ДатаВходящегоДокумента, ДАТАВРЕМЯ(1, 1, 1)),
	|	ЕСТЬNULL(ВТ_СФ.ДатаСФ, ДАТАВРЕМЯ(1, 1, 1)),
	|	ЕСТЬNULL(ДокументПТиУ.НомерВходящегоДокумента, """"),
	|	ВТ_Результат.Документ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВТДанные.Копия_Оригинал КАК Копия_Оригинал,
