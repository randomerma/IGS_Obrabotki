...
Функция СведенияОВнешнейОбработке() Экспорт     
	...
	ПараметрыРегистрации.Вставить("Наименование", "ИНВ-19 (Сличительная ведомость с графами 12-17 регулировка с даты документа инветаризации артикул поставщика) [v.24]");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", ЛОЖЬ);
	ПараметрыРегистрации.Вставить("Версия", "1.0"); 
	ПараметрыРегистрации.Вставить("Информация", ""); 
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд, "ИНВ-19 (артикул поставщика)", "ИНВ19", "открытиеФормы", Истина, "ПечатьMXL");
...
Функция Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода,НачалоКонецДняИнвентаризации, дополнительнаяКолонка) Экспорт
	...
	НомерСтраницы   = 2;
	ПечатьТабличногоДокумента(ТабДокумент, Ложь,	НомерСтраницы, НачалоКонецДняИнвентаризации, дополнительнаяКолонка);
	Возврат ТабДокумент;
	
КонецФункции 

Процедура ПечатьТабличногоДокумента(ТабДокумент, Знач ТолькоОбороты = Ложь, НомерСтраницы, НачалоКонецДняИнвентаризации = Ложь, дополнительнаяКолонка)
	...
	Если дополнительнаяКолонка Тогда
	...
	Запрос.УстановитьПараметр("ДополнительнаяКолонка", дополнительнаяКолонка);	
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА &ДополнительнаяКолонка
	|			ТОГДА Номенклатура1.игсАртикулПоставщика
	|		ИНАЧЕ Номенклатура1.Код
	|	КОНЕЦ КАК ТоварКод,
