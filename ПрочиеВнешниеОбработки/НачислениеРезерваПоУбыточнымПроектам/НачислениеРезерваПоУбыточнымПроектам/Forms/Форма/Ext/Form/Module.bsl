...
&НаСервере
Функция игсПроектЭтапа(Ссылка) 
	ДобавитьРезультат = Ссылка; 
	Проект = Ссылка;
	Пока ЗначениеЗаполнено(ДобавитьРезультат) Цикл
		ДобавитьРезультат = ДобавитьРезультат.Родитель; 
		Если НЕ ЗначениеЗаполнено(ДобавитьРезультат) Тогда
			Возврат Проект;
		КонецЕсли;
		Проект = Проект.Родитель;
	КонецЦикла;
...
Процедура Сформировать(Команда)
	...
	Подразделение = Подразделение(Объект.Организация);
	Статья = Статья(Объект.Организация);
	Счет20 = Счет("20.01");
	Счет96 = Счет("96.09");
...
Функция СоздатьДокумент()
	...
	НовыйДокумент.Дата = ?(ЗначениеЗаполнено(Объект.ДатаФормирования),Объект.ДатаФормирования, ТекущаяДата());
...
&НаСервере
Функция Счет(Счет)
	Возврат ПланыСчетов.Хозрасчетный.НайтиПоКоду(Счет);
КонецФункции 

&НаСервере
Функция Подразделение(Организация)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Настройка", ПланыВидовХарактеристик.игсНастройкиМеханизмов.ПодразделениеНачисленияРезерваПоУбыточнымПроекта);
	...
	Если Результат.Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю("Подразделение не было найдено");
...
&НаСервере
Функция Статья(Организация)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Настройка", ПланыВидовХарактеристик.игсНастройкиМеханизмов.СтатьяЗатратНачисленияРезерваПоУбыточнымПроектам);
